        ...
        for num_sent, tokenized_sentence in enumerate(tokenized_sentences):
            ...
            for pos, token in enumerate(tokenized_sentence):
                for score, candidate in candidates[num_sent][pos]:
                    state = kenlm.State()
                    score = self.left_right_model.BaseScore(
                        prev_state, candidate, state
                    )
                    sentence_candidates_scores[pos].append([score])
                    prev_state = state
            ...
            for inv_pos, token in enumerate(tokenized_sentence[::-1]):
                ...
                for i, (score, candidate) in enumerate(
                    ...
                    sentence_candidates_scores[pos][i].append(score)
                    prev_state = state
            ...
            sentence_candidates_agg_scores = [
                ...
                for pos, scores in sentence_candidates_scores
            ]
            ...
            # select best position and candidates for it
            max_positions_factors = [
                max(factors) for factors in sentence_candidates_scores_factors
            ]
            best_position = max(
                enumerate(max_positions_factors), key=lambda x: x[0]
            )
            ...
            positions_candidates.append(
                [
                    candidates[num_sent][idx] for idx in sort_indices
                ]
            )
